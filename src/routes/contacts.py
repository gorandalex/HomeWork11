...
async def search_contact(
    ...
    contact = await repository_contact.search_contact(first_name=first_name, last_name=last_name, email=email, db=db)
    if contact is None:
...
async def birthday_contact(db: Session = Depends(get_db)):
    contact = await repository_contact.search_birthday_contact(db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not Found")
    return contact
...
async def delete_contact(contact_id: int = Path(1, ge=1), db: Session = Depends(get_db)):
    contact = await repository_contact.delete_owner(contact_id, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not Found")
    return contact
