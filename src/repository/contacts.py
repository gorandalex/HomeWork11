...
from src.models.contacts import Contact, User
...
async def get_contact(contact_id: int, db: Session, owner_id: int) -> Contact:
    contact = db.query(Contact).filter(and_(id=contact_id, owner_id=owner_id)).first()
    return contact
...
async def search_birthday_contact(db: Session, owner_id: int) -> List[Contact]:
